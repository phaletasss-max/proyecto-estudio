...
begin
  ...
  values (
    ...
    coalesce(new.raw_user_meta_data->>'full_name', 'Estudiante ShadowBytes'),
...
revoke all on public.lab_step_secrets, public.lab_hints, public.flag_attempts, public.step_attempts from anon, authenticated;
revoke insert, update, delete on public.lab_steps, public.lab_step_progress, public.user_hint_unlocks from anon, authenticated;
...
begin
  ...
  select s, secret.answer_hash, secret.explanation into v_step, v_hash, v_explanation
  ...
  values (auth.uid(), p_step_id, v_points) on conflict do nothing;
  return query select true, false, v_points, v_explanation;
end;
$$;
...
language plpgsql security definer set search_path = public as $$
declare v_hint public.lab_hints%rowtype;
begin
  if auth.uid() is null then raise exception 'Debes iniciar sesión'; end if;
  if not exists (
...
begin
  ...
  select l.*, s.flag_hash into v_lab, v_expected from public.labs l join public.challenge_secrets s on s.lab_id = l.id where l.id = p_lab_id and l.is_published;
  ...
  if v_lab.is_members_only and v_profile.access_status not in ('member', 'admin') then raise exception 'Completa primero el CTF de admisión'; end if;
  ...
  insert into public.user_solves(user_id, lab_id, points_earned) values (auth.uid(), p_lab_id, v_lab.points)
  on conflict (user_id, lab_id) do nothing;
  v_inserted := found;
  if not v_inserted then return query select true, true, 0, v_profile.points, v_profile.access_status; return; end if;
  ...
  return query select true, false, v_lab.points, v_total, v_next;
end;
$$;
