...
export function Navbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 border-b ${isDark ? 'border-[#222a39] bg-[#07090f]/95' : 'border-slate-200 bg-white/95'} backdrop-blur`}>
      ...
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-2 px-4 sm:gap-4 sm:px-6 lg:px-8">
        <Link to={user ? '/dashboard' : '/'} className="flex shrink-0 items-center gap-3" aria-label="ShadowBytes">
          <img src="/logo-shadowbytes.webp" width="40" height="40" alt="" className="hidden h-10 w-10 border border-purple-500/30 object-cover sm:block" />
          <span className="font-[Orbitron] text-sm font-extrabold tracking-tight sm:text-lg">SHADOW<span className="text-purple-500">BYTES</span></span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button type="button" onClick={() => setSearchOpen(true)} className="hidden min-h-10 items-center gap-2 border border-slate-600 px-3 text-xs sm:inline-flex" aria-label="Buscar contenido (Control K)"><Search size={16} /><kbd className="hidden xl:block">Ctrl K</kbd></button>
          ...
          <UserNavbarBadge />
          <button type="button" onClick={() => setOpen((value) => !value)} className={`inline-flex h-10 w-10 items-center justify-center border xl:hidden ${isDark ? 'border-[#222a39]' : 'border-slate-200'}`} aria-expanded={open} aria-controls="mobile-navigation" aria-label="Abrir navegación">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
        </div>
      </div>

      {open && <nav id="mobile-navigation" aria-label="Navegación móvil" className={`border-t px-4 py-3 xl:hidden ${isDark ? 'border-[#222a39] bg-[#0d111a]' : 'border-slate-200 bg-white'}`}>
        ...
        <div className="mx-auto grid max-w-7xl gap-1">{links.map(({ name, href, icon: Icon }) => <Link key={href} to={href} onClick={() => setOpen(false)} className={`flex min-h-12 items-center gap-3 px-3 text-sm font-semibold ${active(href) ? 'bg-purple-600 text-white' : isDark ? 'text-slate-300 hover:bg-[#121826]' : 'text-slate-700 hover:bg-slate-100'}`}><Icon className="h-4 w-4" />{name}</Link>)}</div>
      </nav>}
